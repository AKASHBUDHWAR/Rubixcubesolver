...
static inline int getIndex(int ind,int row,int col) {
    ...
    void rotateFace(int ind) {
     char tmp[9]={};
    for (int i=ind*9;i<(ind+1)*9;i++) {
        tmp[i%9]=cube[i];
    }
    for (int i=0;i<3;i++) {
        for (int j=0;j<3;j++) {
            cube[ind*9+i*3+j]=tmp[(2-j)*3+i];
        }
    }
...
COLOR getColor(FACE face,unsigned row,unsigned col) const override {
    ...
    bool isSolved() const override {
        for (int i=0;i<54;i++) {
            if (cube[i]!=getColorLetter(COLOR(i/9))) return false;
        }
        return true;
    }
    RubiksCube &u() override {
        this->rotateFace(0);
        char tmp[3]={};
        for (int i=0;i<3;i++) tmp[i]=cube[getIndex(4,0,i)];
        ...
        for (int i=0;i<3;i++) cube[getIndex(3,0,i)]=tmp[i];
        return *this;
}
